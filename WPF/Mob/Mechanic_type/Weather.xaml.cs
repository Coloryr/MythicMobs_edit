...
namespace MythicMobs_edit.WPF.Mob.Mechanic_type
{
    ...
    public partial class Weather : Window
    {
        ...
        public Weather(Mechanic Mechanic)
        {
            InitializeComponent();
            this.Mechanic = Mechanic;
            if (!(this.Mechanic.Option is Obj_save.Mob.Mechanic_type.Weather))
            {
                this.Mechanic.Option = new Obj_save.Mob.Mechanic_type.Weather();
            }
            ...
            DataContext = this;
            Type_.ItemsSource = new List<string>()
            {
                "sunny", "rainy", "stormy"
            };
