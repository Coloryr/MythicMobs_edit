...
namespace MythicMobs_edit.WPF.Mob.Mechanic_type
{
    ...
    public partial class Velocity : Window
    {
        ...
        public Velocity(Mechanic Mechanic)
        {
            InitializeComponent();
            this.Mechanic = Mechanic;
            if (!(this.Mechanic.Option is Obj_save.Mob.Mechanic_type.Velocity))
            {
                this.Mechanic.Option = new Obj_save.Mob.Mechanic_type.Velocity();
            }
            ...
            DataContext = this;
            Type_.ItemsSource = new List().Modify_;
